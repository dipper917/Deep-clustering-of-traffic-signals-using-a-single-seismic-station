...
ts = np.zeros([1, ns])  # Source delay time
...
for i in range(500):
    ...
    V_bike = 5 + 15 * random.random()  # Bicycle speed (m/s)
    ts[0, 0] = 4.5 * random.random()

    # Single axle source function
    wavelet = Ricker(fm, dt)
    waveletnt = np.size(wavelet)

    # Dual-axle source function
    D_train = np.array([0, 1])  # Wheel positions
    Dnt = np.zeros([1, 2])
    for i in range(2):
        Dnt[0, i] = math.floor(D_train[i] / V_bike / dt)
    ...
    # St = uxt / (uxt.max(axis=0))  # With normalization

    # Add white noise
    sigma = 0.05
    St = St + sigma * 10 ** 3 * np.random.randn(St.shape[0], St.shape[1])

    # Optionally save data
    # np.save(file=os.path.join('data_pre/train data', 'image(bike)%d.npy' % (aa + 1)), arr=St)
    # aa = aa + 1
