...
for i in range(5000):
    ...
    for i in range(ns):
        ts[0, i] = 5 * random.random()
    for i in range(nr):
        for k in range(ns):
            D[k, i] = ((rx - sx[0, k]) ** 2 + (ry - sy[0, k]) ** 2) ** 0.5
    ...
    #np.save(file=os.path.join('/data_pre/train data/', 'image(ambient noise)%d.npy' % (aa+1)), arr=St)
    aa = aa + 1

    ## Visualization
    # Visualize one example of seismic data
